...
-----------------------------------------------------------------------------}
// $Id: JvSimpleXml.pas,v 1.2 2005/08/27 10:22:42 dcoder Exp $
      ...
      case lPos of
        1..4: //<!--
          if lBuf[I] = CS_START_COMMENT[lPos] then
            ...
            FmtError('Error', [CS_START_COMMENT[lPos], lBuf[I]]);
        5:
      ...
      case lPos of
        1..9: //<![CDATA[
          if lBuf[I] = CS_START_CDATA[lPos] then
            ...
            FmtError('Error', [CS_START_CDATA[lPos], lBuf[I]]);
        10:
      ...
      case lPos of
        1..4: //<?xml
          if lBuf[I] = CS_START_HEADER[lPos] then
            ...
            FmtError('Error', [CS_START_HEADER[lPos], lBuf[I]]);
        5: //L
          if lBuf[I] = CS_START_HEADER[lPos] then
          begin
            Stream.Seek(lStreamPos, soFromBeginning);
        ...
        6: //?
          if lBuf[I] = CS_STOP_HEADER[lPos] then
            ...
            FmtError('Error', [CS_STOP_HEADER[lPos], lBuf[I]]);
        7: //>
          if lBuf[I] = CS_STOP_HEADER[lPos] then
          begin
            Count := 0; //End repeat
      ...
      case lPos of
        1..9: //<!DOCTYPE
          if lBuf[I] = CS_START_DOCTYPE[lPos] then
            ...
            FmtError('Error', [CS_START_DOCTYPE[lPos], lBuf[I]]);
        10: //]> or >
      ...
      case lPos of
        1..15: //<?xml-stylesheet
          if lBuf[I] = CS_START_PI[lPos] then
            ...
            FmtError('Error', [CS_START_PI[lPos], lBuf[I]]);
        16: //L
          if lBuf[I] = CS_START_PI[lPos] then
          begin
            Stream.Seek(lStreamPos, soFromBeginning);
        ...
        17: //?
          if lBuf[I] = CS_STOP_PI[lPos] then
            ...
            FmtError('Error', [CS_STOP_PI[lPos], lBuf[I]]);
        18: //>
          if lBuf[I] = CS_STOP_PI[lPos] then
          begin
            Count := 0; //End repeat
