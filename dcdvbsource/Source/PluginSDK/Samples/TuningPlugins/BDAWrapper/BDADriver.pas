...
begin
  case AMessage of
    ...
    DCDVB_TUNING_PLUGIN_CTRL_PROGRAM_CHANGED:
    begin
      Result := S_OK;
    end;
    DCDVB_TUNING_PLUGIN_CTRL_PMT_BUFFER:
    begin
      Result := S_OK;
    end;
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                if FTuneDVBS2
                  then mod_type := BDA_MOD_8VSB
                  else mod_type := ModulationType(BDA_MOD_NOT_SET);
                  hr := put_ModulationType(mod_type);
                Log('TuneRequest', 'DVBs - put_ModulationType returned ' + inttohex(hr, 8));
              end;
            end;
          ...
          begin
            ...
            begin
              ...
              if FTuneDVBS2
                then hr := put_Modulation(ModulationType(BDA_MOD_8VSB))
                else hr := put_Modulation(ModulationType(BDA_MOD_NOT_SET));
              Log('TuneRequest', 'DVBs - locator::put_Modulation returned ' + inttohex(hr, 8));
...
begin
//OutputDebugString(PChar('TS DATA ' + inttostr(ASize) + ' bytes -> ' + inttohex(PCardinal(ABuffer)^, 8)));
  if not FPushData
    then Exit;
...
function TBDADevice.get_DirectTuning(out ADirectTuning: BOOL): HRESULT;
begin
...
function TBDADevice.put_DirectTuning(ADirectTuning: BOOL): HRESULT;
begin
  FDirectTuning := ADirectTuning;
  Result := S_OK;
end;

function TBDADevice.get_TuneDVBS2(out ADVBS2: BOOL): HRESULT;
begin
  if not Assigned(@ADVBS2) then
  begin
    Result := E_POINTER;
    Exit;
  end;
...
function TBDADevice.put_TuneDVBS2(ADVBS2: BOOL): HRESULT;
begin
  FTuneDVBS2 := ADVBS2;
  Result := S_OK;
end;
